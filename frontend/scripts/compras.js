...
function atualizarSelectFornecedores() {
  ...
  fornecedoresCache =
    fornecedoresCache.filter(isFornecedor);
  ...
  tr.querySelector(".item-produto")
    .addEventListener(
      ...
      (event) => {
        ...
        tr.querySelector(".item-custo").value =
          option?.dataset?.custo || 0;

        recalcularTotais();
      }
    );

  tr.querySelector(".item-qtd")
    .addEventListener(
      "input",
      recalcularTotais
    );

  tr.querySelector(".item-custo")
    .addEventListener(
      "input",
      recalcularTotais
    );
...
function limparFormulario() {
  ...
  controlarCamposPrazo();
}

function controlarCamposPrazo() {
  const forma =
    document.getElementById(
      "forma_pagamento"
    )?.value;
...
async function carregarBase() {
  const [produtos, entidades] =
    await Promise.all([
      API.get("/api/produtos"),
      API.get("/api/entidades")
    ]);

  produtosCache = Array.isArray(produtos)
    ? produtos.filter(
        (p) => p.ativo !== false
      )
    : [];
      ...
      comprasCache.map((compra) => {
        ...
        return `
          <tr>
            ...
            <td>
              ${nomeEntidade(compra.fornecedor)}
            </td>
                ...
                podeConfirmar
                  ? `
                    <button
                      ...
                      onclick="confirmarCompra(${compra.id})"
                    >
                      Confirmar
                    </button>
                  `
    ...
    tbody.innerHTML =
      lista.map((mov) => `
        <tr>
          ...
          <td>
            ${mov.origem} #${mov.origem_id || "-"}
          </td>

          <td>
            ${numberBR(mov.quantidade)}
          </td>

          <td>
            ${numberBR(mov.saldo_anterior)}
          </td>

          <td>
            ${numberBR(mov.saldo_novo)}
          </td>
        </tr>
      `).join("");
...
async function salvarCompra(event) {
  ...
  if (!itens.length) {
    status.textContent =
      "Adicione pelo menos um item válido na compra.";
...
async function confirmarCompra(id) {
  ...
  const nf =
    compra?.nf_entrada ||
    prompt(
      "Informe a NF de entrada (opcional):"
    ) ||
    "";
  ...
  try {
    ...
    alert(
      "Compra confirmada. Estoque atualizado automaticamente."
    );
...
document.addEventListener(
  ...
  () => {
    ...
    document
      .getElementById("limpar-form-button")
      .addEventListener(
        "click",
        limparFormulario
      );
